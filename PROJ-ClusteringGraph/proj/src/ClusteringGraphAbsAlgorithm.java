 ...
 * @author ...
 */
public abstract class ClusteringGraphAbsAlgorithm extends AbsAlgorithm {

  ClusteringGraphTestCase clusteringTestCase;
  int [] resultC;
  @Override
  public ErrorStatus init(TestCase test) {
    if (test instanceof ClusteringGraphTestCase) {
      clusteringTestCase = (ClusteringGraphTestCase) test;
      int n = clusteringTestCase.graph != null ? clusteringTestCase.graph.length : 0;
      resultC = new int[n];
  ...
  public void run() {
    ...
    execute(clusteringTestCase.graph, resultC);
  }
